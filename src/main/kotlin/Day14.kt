...
fun main() {
    Day14.solve(bothParts = true, checkPart1 = 1588L, checkPart2 = 2188189693529L)
}

object Day14 : Aoc2021(production = true, debug = false) {
...
//        .joinToString(separator = "") {
    ...
    private fun solveTier5(template: String, levels: Int, count: Int): Map<Char, Long> {
        val steps = count
...
//        repeat(10) { step() }; return result // Wrong way
//        return solveRecursive(inputData, 10) // Also wrong

//        val result = processSteps(inputData, 10)
//        val groups = result.groupByFirstLetterAndCount()

        val groups = solveTier5(inputData, 10, 1)

        return groups.maxMinusMin()
    }

    override fun solvePart2(debug: Boolean): Long {
//        return solveRecursive(inputData, 40)
//        repeat(10) { step() } return result  // 40 times -> 10 from previous part + 30 more

        val groups = solveTier5(inputData, 20, 2)
